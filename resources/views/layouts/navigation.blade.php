<nav :class="minimized ? 'w-20 px-2 py-5' : 'w-72 p-5'" x-init="
        minimized = window.innerWidth <= 1024;
        window.addEventListener('resize', () => {
            minimized = window.innerWidth <= 1024;
        });
     "
    class="flex flex-col justify-between bg-white border-r border-zinc-300 top-0 left-0 fixed z-[9999] h-screen transition-all duration-300">

    <div class="space-y-4">

        <div class="flex justify-between items-center pb-2 border-b border-zinc-300">
            <span x-show="!minimized" x-transition>
                <x-application-logo />
            </span>

            <button @click="minimized = !minimized"
                :class="minimized ? 'text-center justify-self-center self-center w-full' : ''"
                class="flex justify-center items-center p-2 rounded-md hover:bg-zinc-100 transition">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor"
                    :class="minimized ? 'size-6 text-center' : 'size-5'" aria-hidden="true">
                    <path
        ...
        <div class="flex flex-col gap-3">
            <x-nav-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard', '/')">
                <svg xmlns="http://www.w3.org/2000/svg"
                    :class="minimized ? 'size-6 text-center justify-self-center w-full' : 'size-5'" fill="currentColor"
                ...
                <span x-show="!minimized" x-transition class="leading-none">{{ __('Dashboard') }}</span>
            </x-nav-link>

            <div>
                <span x-show="!minimized" x-transition><x-nav-label>Data sekolah</x-nav-label></span>
                <div class="space-y-1">

                    <x-nav-link href="{{ route('tahun_ajar.index') }}" :active="request()->routeIs('tahun_ajar.index')">
                        <svg xmlns="http://www.w3.org/2000/svg"
                            :class="minimized ? 'size-6 text-center justify-self-center w-full' : 'size-5'"
                            fill="currentColor"
                            viewBox="0 0 640 640"><!--!Font Awesome Free v7.1.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.-->
                            <path
                        ...
                        <span x-show="!minimized" x-transition class="leading-none">{{ __('Tahun ajar') }}</span>
                    </x-nav-link>
                    ...
                    <x-nav-link href="{{ route('jurusan.index') }}" :active="request()->routeIs('jurusan.index')">
                        ...
                        <span x-show="!minimized" x-transition class="leading-none">{{ __('Jurusan') }}</span>
                    </x-nav-link>
                    ...
                    <x-nav-link href="{{ route('kelas.index') }}" :active="request()->routeIs('kelas.index')">
                        ...
                        <span x-show="!minimized" x-transition class="leading-none">{{ __('Kelas') }}</span>
                    </x-nav-link>
            ...
            <div>
                <span x-show="!minimized" x-transition><x-nav-label>Manajemen siswa</x-nav-label></span>
                <div class="space-y-1">
                    ...
                    <x-nav-link href="{{ route('siswa.index') }}" :active="request()->routeIs('siswa.index')">
                        ...
                        <span x-show="!minimized" x-transition class="leading-none">{{ __('Daftar siswa') }}</span>
                    </x-nav-link>
            ...
            <div>
                <span x-show="!minimized" x-transition><x-nav-label>User sistem</x-nav-label></span>
                <div class="space-y-1">
                    ...
                    <x-nav-link href="{{ route('users.index') }}" :active="request()->routeIs('users.index')">
                        ...
                        <span x-show="!minimized" x-transition class="leading-none">{{ __('Manajemen user') }}</span>
                    </x-nav-link>
    ...
    <div :class="minimized ? 'text-center justify-center' : 'justify-between'"
        class="flex items-center p-2 w-full h-fit border border-zinc-300 rounded-md">
        <div class="flex gap-2 items-center">
            <div x-show="!minimized" x-transition class="flex flex-col justify-center">
                <strong class="font-medium leading-none">{{ Auth::user()->name }}</strong>
        ...
        <div x-data="{ open: false }" class="relative">
            ...
            <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-300"
                ...
                :class="minimized ? '' : '-right-2'"
                class="bottom-16 absolute w-[246px] bg-gradient-to-tr from-zinc-100 to-white border border-zinc-300 shadow-md rounded-xl p-4 z-50"
